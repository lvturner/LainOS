...
On the host, profiles are at `config/lain/profiles/<name>/` (bind-mounted into the container).
...
```bash
mkdir -p config/lain/profiles/codellama
cat > config/lain/profiles/codellama/config.yaml << 'EOF'
...
cat > config/lain/profiles/codellama/servers.json << 'EOF'
{ "mcpServers": {} }
EOF

cat > config/lain/profiles/codellama/agents.md << 'EOF'
